const messages = {
  errors: {
    disconnected: () =>
      'Dekey: Disconnected from chain. Attempting to connect.',
    permanentlyDisconnected: () =>
      'Dekey: Disconnected from Dekey background. Page reload required.',
    sendSiteMetadata: () =>
      `Dekey: Failed to send site metadata. This is an internal error, please report this bug.`,
    unsupportedSync: (method: string) =>
      `Dekey: The Dekey Ethereum provider does not support synchronous methods like ${method} without a callback parameter.`,
  ...
  info: {
    connected: (chainId: string) =>
      `Dekey: Connected to chain with ID "${chainId}".`,
  },
  warnings: {
    // deprecated methods
